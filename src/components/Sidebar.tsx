...
import { ChevronDown, Loader2, Eye, EyeOff, Droplets, Camera, X, Copy, Video, Play, ChevronRight, Sparkles, Spline, Scissors, ZoomIn, ZoomOut, Compass, RotateCcw } from 'lucide-react';
    ...
    <aside className={`transition-all duration-300 bg-zinc-50 dark:bg-zinc-900 flex-col overflow-y-auto overflow-x-hidden shrink-0 ${collapsed ? 'w-full md:w-0 h-0 md:h-auto opacity-0 border-none pointer-events-none' : 'w-full md:w-[280px] h-auto max-h-[50vh] md:max-h-none md:h-auto border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-800 flex'}`}>
      <div className="flex flex-col w-full min-h-min pb-5">
        ...
        {(!isEmpty && !status.includes('Loading') && !status.includes('Parsing') && !status.includes('Error')) ? null : (
          <div className="p-6">
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded p-4 text-[13px] text-zinc-600 dark:text-zinc-400 text-center shadow-sm">
              <div className="flex items-center gap-1.5 mb-2.5 text-[10px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 font-mono justify-center">
                <span className="relative flex h-2 w-2">
                  ...
                  <span className={`relative inline-flex rounded-full h-2 w-2 ${
                    ...
                    (status.includes('Loading') || status.includes('Parsing')) ? 'bg-blue-500' : 'bg-zinc-400'
                  }`}></span>
                </span>
              ...
              <div className="text-zinc-700 dark:text-zinc-300">
                {status.includes('Loading') || status.includes('Parsing') ? (
                  <div className="flex flex-col items-center justify-center text-blue-600 dark:text-blue-500 w-full mt-2">
                    ...
                    <span className="text-zinc-800 dark:text-zinc-200 whitespace-pre-line text-xs font-semibold">{status.replace(/\*\*/g, '')}</span>
        ...
        <div className="flex justify-between items-center cursor-pointer px-6 py-4 text-[11px] font-bold uppercase tracking-[0.05em] text-zinc-400 border-t border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800" onClick={() => setMeshVisOpen(!meshVisOpen)}>
        ...
        {meshVisOpen && (
          <div className="px-6 py-4">
            <div className="border-l-2 border-zinc-900 dark:border-zinc-100 pl-3 mb-6">
              <label className="block text-[10px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mb-2 font-semibold">Global Opacity</label>
                ...
                meshes.map((mesh) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <div className="flex items-center gap-0.5">
                        <button 
                          ...
                          title="Add / Duplicate to Object List & Planning Tools"
                        >
                          <Copy size={13} />
                        </button>
                        <button 
                    ...
                    <div className="flex items-center gap-2.5 opacity-80" onClick={e => e.stopPropagation()}>
                      <Droplets size={12} className="text-zinc-400" title="Opacity" />
                      <input 
                        type="range" 
                        min="0" max="1" step="0.05" 
                        value={mesh.opacity} 
        ...
        <div className="flex justify-between items-center cursor-pointer px-6 py-4 text-[11px] font-bold uppercase tracking-[0.05em] text-zinc-400 border-t border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800" onClick={() => setAdvancedOpen(!advancedOpen)}>
        ...
        {advancedOpen && (
          <div className="px-6 py-4 flex flex-col gap-6">
            ...
            <div className={`border-l-2 ${splineClippingState.active ? 'border-indigo-500 bg-indigo-950/20 rounded-r p-2.5 -ml-1 pl-3' : 'border-indigo-500 pl-3'} transition-all`}>
            ...
            <div className={`border-l-2 ${flyThroughState.active ? 'border-cyan-500 bg-cyan-950/20 rounded-r p-2.5 -ml-1 pl-3' : 'border-cyan-500 pl-3'} transition-all`}>
              <div className="flex justify-between items-center mb-2">
                <div 
                  className="flex items-center gap-2 cursor-pointer flex-1"
